...
export function parseIngCsv(text: string, filename: string): ParsedImport {
  ...
  const periodEnd = parseGermanDate(period[2]!)
  ...
  if (!header) throw new Error('Keine ING-Umsatztabelle gefunden.')

  const hasBalance = headersEqual(header, headersWithBalance)

  if (!hasBalance && !headersEqual(header, headersWithoutBalance))
  ...
  const transactions = result.data.map((r, i) => {
    try {
      if (r.length !== header.length) throw new Error('Unerwartete Spaltenanzahl.')
