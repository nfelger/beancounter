...
- Financial data normally stays in memory. An unconfirmed import or assignment-and-rule save may be held in tab-scoped sessionStorage for recovery, including the exact private rule rows needed to retry; clear it immediately after verified success. Do not add general persistent financial caching. OAuth access tokens may be saved in localStorage with their OAuth client, scope, and absolute expiry; restore only valid matching grants, clear on expiry/rejection/disconnect, and propagate disconnects across tabs. Never persist refresh tokens or client secrets.
